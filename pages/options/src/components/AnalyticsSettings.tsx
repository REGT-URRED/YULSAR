      ...
      <section className="space-y-6">
        <div
          ...
          <div className="animate-pulse">
            <div className={`mb-2 h-4 w-3/4 rounded ${isDarkMode ? 'bg-onyx-800' : 'bg-gray-200'}`}></div>
            <div className={`h-4 w-1/2 rounded ${isDarkMode ? 'bg-onyx-800' : 'bg-gray-200'}`}></div>
          </div>
        </div>
      </section>
    );
    ...
    return (
      <section className="space-y-6">
        <div
          ...
          <p className={`${isDarkMode ? 'text-crimson-400' : 'text-red-600'}`}>Failed to load analytics settings.</p>
        </div>
      </section>
    );
  }
      ...
      <div
        ...
        <div className="space-y-6">
          ...
          <div
              ...
              <div className="relative inline-block w-12 select-none">
                ...
                <label
                  ...
                  className={`block h-6 cursor-pointer overflow-hidden rounded-full ${
                    settings.enabled ? 'bg-blue-500' : isDarkMode ? 'bg-onyx-500' : 'bg-gray-300'
                  }`}>
            ...
            <p className={`mt-2 text-sm ${isDarkMode ? 'text-bone-400' : 'text-bone-600'}`}>
          ...
          <div
            ...
            <h3 className={`text-base font-medium ${isDarkMode ? 'text-bone' : 'text-bone-700'} mb-4`}>
              What we collect:
            </h3>
            <ul
              className={`list-disc space-y-2 pl-5 text-left text-sm ${isDarkMode ? 'text-bone-300' : 'text-bone-600'}`}>
            ...
            <h3 className={`mb-4 mt-6 text-base font-medium ${isDarkMode ? 'text-bone' : 'text-bone-700'}`}>
          ...
          {!settings.enabled && (
            <div
                className={`rounded-md border p-4 ${isDarkMode ? 'border-bone-700 bg-bone-900/20' : 'border-yellow-200 bg-yellow-50'}`}>
              <p className={`text-sm ${isDarkMode ? 'text-bone-300' : 'text-yellow-700'}`}>
                 Analytics disabled. You can re-enable it anytime to help improve YULSAR.
              </p>
            </div>
          )}
        </div>
